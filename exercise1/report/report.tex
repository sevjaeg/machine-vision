...
\subcaption{Blurred ($\sigma=3$)}
\end{subfigure}
\caption{Output images of the non-maximum suppression algorithm with different input images.}
\label{non_maximum}
\end{figure}
...
\end{subfigure}
\caption{Images (top) and detected edges (bottom) for Gaussian noise added to the input images.}
\label{noise}
\end{figure}

\subsection{Alleviating the influence of noise}

\subsubsection*{Altering $\sigma$}
...
\subcaption{$\sigma=4$}
\end{subfigure}
\caption{TODO} % sigma_noise, t1, t2}
\label{noise_sigma}
\end{figure}

\subsubsection*{Altering the hysteresis thresholds}
