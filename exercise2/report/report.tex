...
\label{sec:rot}
Rotation is a major challenge for matching. Figure \ref{rotation} depicts the response of the basic descriptor matching for different rotations. Small rotations (around \SI{10}{\degree}) do not deteriorate the performance significantly, however for larger angles (around \SI{30}{\degree}) the matching does not deliver satisfactory results any longer. This is reasonable, as the descriptor uses the intensities of the neighbouring pixels, in case of rotation those are at different positions.
...
The normalised descriptor is helpful with different lighting conditions. This is due to the fact that changing light conditions imply different behaviour of the gradients, therefore the values of the neighbouring pixels diverge. Nonetheless, their relations remain widely unchanged, so normalising alleviates this problem. This can be observed in Figure \ref{norm}.
...
However, this increases the rotation variance of the descriptor as in case of a rotated image not only the direction values change, but also the block to which a pixel belongs might be altered. Furthermore, the smaller blocks counteract the aforementioned advantage of outlier suppression via the histograms.
